...
networkAddressForClient = HOST + PORT.__str__()
BLOCK_SIZE = 64
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s1:
    ...
    s1.bind((HOST, PORT1))  # binds server socket to address
    ...
    with client:
        ...
        with open('clientKey.txt') as key:  # encrypt message to be sent back using Client key
            string = key.read(8)
            string1 = string
            cipher = DES.new(std_serverKey.encode('utf-8'), DES.MODE_ECB)
            data = client.recv(1024)
            dec = data.decode()
            print(dec[2:320])
            unpaddedData = unpad(data[2:132], BLOCK_SIZE)
            decryptedData = cipher.decrypt(data)  # stores decoded byte data as string
            dataUnpadded = unpad(decryptedData, BLOCK_SIZE)
            decodedData = dataUnpadded.decode('utf-8').strip()

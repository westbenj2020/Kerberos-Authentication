...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:  # creates server socket
    ...
    with client:
        ...
        with open('clientKey.txt') as key:  # encrypt message to be sent back using Client key
            ...
            authenticatorData = {receivedDataDecoded.replace(std_server_id, '').replace(nonencryptedticketTGS, '')}
            strAuthenticatorData = authenticatorData.__str__()
            strAuthEncoded = strAuthenticatorData.encode('utf-8')
            ...
            strAuthDataDecrypted = cipherAuthenticator.decrypt(strAuthEncoded, BLOCK_SIZE)
            timestamp3 = {authenticatorDataDecoded.replace(string1, '').replace(networkAddress, '')}
            ts3Str = timestamp3.__str__()
            ts3Int = int(ts3Str)
            currentUnixTime = time.time()
            if currentUnixTime - ts3Int < lifetime2:
